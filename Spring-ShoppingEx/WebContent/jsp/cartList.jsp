 ...
 */
 ...
 section#content div.gdsInfo .delete button { font-size:18px;
...
.delBtn button { font-size:18px; padding:4px 10px; border:1px solid #eee; background:#eee;}
...
<div id="root">
    ...
    <%@ include file ="aside.jsp" %>
          ...
          <section id="content">
			<ul>
			     ...
			     <input type="checkbox" name="allCheck" id="allCheck"/><label for="allCheck">모두 선택</label>
				      ...
				      <script>
				       ...
				       $("#allCheck").click(function() {
						...
						var chk = $(".allCheck input").prop("checked");
			     ...
			     <button type="button" class="selectDelete_btn">선택 삭제</button>
                    ...
                    $(".selectDelete_btn").click(function(){
                    	  ...
                    	  if(confirm_val) {
                    	   ...
                    	   $.ajax({
                    	    url : "/Spring-ShoppingEx/deleteCart.do",
                    	    type : "post",
                    	    dataType: 'json',
                    	    traditional: true,
                    	    data : {'chBox[]' : checkArr},
                    	    success : function(result){
                    	    	if(result == 1){
                    	            ...
                    	            location.href = "/Spring-ShoppingEx/cartList.do";
                    	    	}else{
                    	    		alert("삭제실패");
                    	    	}
			   ...
			   <c:forEach items="${cartList}" var="clist">
				  <li>
				  <div class="checkBox">
				    <input type="checkbox" name="chBox" id="chBox" data-cartNum="${clist.cartNum }">
					...
					<div class="thumb">
					  <img src="/Spring-ShoppingEx${clist.gdsThumbImg}">
				   </div>
					<div class="gdsInfo">
								
					<p>
				      <span>상품명 :</span>${clist.gdsName }<br/>
					  <span>개당 가격 :</span>
					  <fmt:formatNumber pattern="###,###,###" value="${clist.gdsPrice }"/>원<br/>
					  <span>구입 수량 :</span> ${clist.cartStock }개<br/>
				      <span>최종 가격 :</span> 
					  <fmt:formatNumber pattern="###,###,###" value="${clist.gdsPrice * clist.cartStock }"/>원			
					</p>
					<div class="delete">
					   
					   <button type="button" class="delete_${clist.cartNum}_btn" data-cartNum="${clist.cartNum }">제거</button>
					      <script>
					      
					       $(".delete_${clist.cartNum}_btn").click(function() {
							 var confirm_val = confirm("정말 삭제하실래요?");
				...
				<c:set var="sum" value="${sum + (clist.gdsPrice * clist.cartStock)}" />
			</c:forEach>
		</ul>
		<div class="listResult">
		   ...
		   <div class="orderOpen">
		     <button type="button" class="orderOpen_btn">주문정보입력</button>
		     <script>
		     $(document).on('click',".orderOpen_btn", function () {
		      ...
		      /*     $(".orderOpen_btn").on('click' ,function () {
					...
					<div class="orderInfo">
						<form action="cartList.do" method="post" autocomplete="off">
